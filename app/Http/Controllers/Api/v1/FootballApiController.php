...
class FootballApiController extends Controller {
    ...
    public function match_schedules(Request $request) {
      ...
      $html = file_get_html('https://www.espn.in/football/fixtures/_/date/' . $request->date);
	...
	public function bingsport_m3u8(Request $request)  {
		...
		foreach($m3u8Match as $m3u8Val) {
			...
			//m3u8 link 2
			$l2 = $m3u8Val->find('.item-server', 1)->{'data-link'} ?? '';
			$l2_01 = explode('m3u8=', $l2);
			$l2_02 = $this->hpt421($l2_01[1]);
			$l2_03 = explode('&', $l2_02);
			$link2 = $l2_03[0];

			$m3u8Object = array('access-control-allow-origin' => '*', 'referer' => 'https://live-streamfootball.com/'); 
			$headers = $this->a_to_s($m3u8Object);
			$m3u8Array[] = array('name' => 'Server 1', 'link' => $link1, 'header' => $headers); 
			$m3u8Array[] = array('name' => 'Server 2', 'link' => $link2, 'header' => $headers);
		}
	...
	public function hpt421($str){
        $str = str_replace('%3A%2F%2F', '://', $str);
        $str = str_replace('%2F', '/', $str);
        $str = str_replace('index', 'bingsport', $str);
        return $str;
    }
