<?php
    ...
    switch ($request) {
        case 'POST':
                ...
                if($data["api_key"] == "Nf8vf7766bb683c1e785rEx34783f5318665b73c8d021424"){
        ...
        foreach($liveMatchs as $article) {
            ...
            foreach ($article->find('.match-row') as $value) {
                ...
                if (hpt($status) == "Scheduled" || hpt($status) == "Live") {
					if ($teamName1 != "TBA") {
						$test_array[] = array('match-id' => $link, 'status' => $status, 'day' => $day, 'vanue' => hpt($vanue), 'subtitle' => $subtitle, 'series_name' => hpt($title), 'time' => $time, 'localTime' => $localTime, 'startat' => $startat, 'image1' => $rexImage1, 'teamName1' => hpt($teamName1), 'image2' => $rexImage2, 'teamName2' => hpt($teamName2));
					}
                }
