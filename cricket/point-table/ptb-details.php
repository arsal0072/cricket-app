<?php
   ...
   function postRequestCapture($data){
        ...
        $ptbHtml2 = $pointstable->find('.cb-srs-gray-strip');
        $count = count($ptbHtml2);
        $dCount = 1;
        $groupIndex = 0;
        $groupArray = array();
        ...
        if ($count != $dCount) {
            ...
            foreach($ptbHtml2 as $groupArt) {

                $gNames = $groupArt->find('th', 0);
                $names = $gNames->innertext;

                $groupArray[] = array('id' => $groupIndex, 'name' => $names);
                $groupIndex++;
                
            }

        } else {
            $ptvListHtml2 = $pointstable->find('table');
            foreach($ptvListHtml2 as $singleArt) {

                foreach ($singleArt->find('tbody tr') as $value) {
        ...
        $object = array('value'=>$count, 'group'=>$groupArray, 'single'=>$singleArray);
